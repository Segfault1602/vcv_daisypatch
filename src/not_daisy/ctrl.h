...
class AnalogControl
{
  public:
    ...
    \param *adcptr is a pointer to the raw adc read value -- This can be acquired with dsy_adc_get_rawptr(), or
    dsy_adc_get_mux_rawptr() \param sr is the samplerate in Hz that the Process function will be called at. \param flip
    determines whether the input is flipped (i.e. 1.f - input) or not before being processed.1 \param invert determines
    whether the input is inverted (i.e. -1.f * input) or note before being processed. \param slew_seconds is the slew
    time in seconds that it takes for the control to change to a new value.
    */
    void Init(float* adcptr, float sr, bool flip = false, bool invert = false, float slew_seconds = 0.002f);
    ...
    */
    void InitBipolarCv(float* adcptr, float sr);
    ...
    */
    ...
    inline uint16_t GetRawValue()
    {
        return (uint16_t)(*raw_ * 65535.f);
    }
